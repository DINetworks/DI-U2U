...
function generateMockCrossChainRoute(
  ...
  const sourceDexRoutes = MOCK_DEXES.slice(0, 15).map((dex) =>
    generateMockDexRoute(
      sourceToken.address,
      IU2U_ADDRESSES[sourceChain.id],
      request.amount,
      dex,
    ),
  );
  ...
  const destinationDexRoutes = MOCK_DEXES.slice(15, 30).map(
    (dex) =>
      generateMockDexRoute(
        IU2U_ADDRESSES[destinationChain.id],
  ...
  const totalGasCost = (
    parseFloat(bestSourceRoute.estimatedGas) +
    parseFloat(bestDestinationRoute.estimatedGas) +
    150000
  ).toString();
...
export async function getSwapRoutes(
  ...
  try {
    ...
    const destinationChain = mockChains.find(
      (c) => c.id === request.destinationChainId,
    );
    const sourceToken = mockTokens.find(
      (t) => t.address === request.sourceTokenAddress,
    );
    ...
    routes.sort(
      (a, b) =>
        parseFloat(b.destinationDexRoute.amountOut) -
        parseFloat(a.destinationDexRoute.amountOut),
    );

    const bestRoute = routes[0];
    const exchangeRate = (
      parseFloat(bestRoute.destinationDexRoute.amountOut) /
...
export async function executeSwap(): Promise<ExecuteResponse> {
  try {
    // Simulate API delay
    await new Promise((resolve) =>
