...
export interface UseBridgeTransactionsReturn {
  ...
  // Actions
  addTransaction: (transaction: Omit<BridgeTransaction, 'id' | 'timestamp'>) => string;
  ...
  executeSendToken: (destinationChain: Chain, recipientAddress: string, bridgeAmount: string) => Promise<void>;
  executeContractCall: (
...
export function useBridgeTransactions(): UseBridgeTransactionsReturn {
  ...
  const addTransaction = useCallback((transactionData: Omit<BridgeTransaction, 'id' | 'timestamp'>) => {
  ...
  const addCrossChainTransaction = useCallback((tx: { id: string; commandId: string; destinationChain: string }) => {
  ...
  const handleTransactionError = (error: any, operation: string) => {
    if (error?.name === 'ContractFunctionExecutionError' &&
      ...
      console.log(`User rejected ${operation} transaction`);
  ...
  const executeSendToken = useCallback(async (
    destinationChain: Chain,
    recipientAddress: string,
    bridgeAmount: string
  ) => {
    try {
      const destinationChainName = getChainNameForGateway(destinationChain.id);
