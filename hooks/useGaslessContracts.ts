...
export const useGaslessContracts = () => {
  ...
  const initializeContracts = useCallback(async () => {
    ...
    try {
      ...
      setError(`Contract validation failed: ${err instanceof Error ? err.message : 'Unknown error'}`)
    } finally {
      setIsLoading(false)
    }
  }, [vaultPublicClient])
  ...
  const getBatchTransactionLog = useCallback(
    async (batchId: bigint): Promise<readonly [string, bigint, bigint, boolean]> => {
      if (!publicClient) throw new Error('Public client not available')
  ...
  const generateEIP712Signature = useCallback(
    async ({ metaTxs, deadline }: { metaTxs: any[], deadline: number }): Promise<string> => {
  ...
  const estimateTransfer = useCallback(
    async ({
      ...
      const params = {
        chainId: chain.id,
        from: owner,
        metaTxs: metaTxs.map((tx: any) => ({ ...tx, value: tx.value.toString() })),
  ...
  const getApprovedAmount = useCallback(
    async (token: Address): Promise<bigint> => {
      if (!vaultPublicClient) throw new Error('Vault public client not available')
  ...
  const getWhitelistedTokens = useCallback(async (): Promise<readonly Address[]> => {
    if (!vaultPublicClient) throw new Error('Vault public client not available')
    return await vaultPublicClient.readContract({
      address: vaultAddress,
      abi: VAULT_ABI,
  ...
  const getWhitelistedRelayers = useCallback(async (): Promise<readonly Address[]> => {
    if (!vaultPublicClient) throw new Error('Vault public client not available')
    return await vaultPublicClient.readContract({
      address: vaultAddress,
      abi: VAULT_ABI,
