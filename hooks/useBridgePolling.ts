...
import { checkCommandStatus, extractCommandIdFromReceipt, getChainNameForGateway } from "@/utils/bridge";
...
export interface UseBridgePollingProps {
  ...
  crossChainTransactions: { id: string; commandId: string; destinationChain: string }[];
  ...
  addCrossChainTransaction: (tx: { id: string; commandId: string; destinationChain: string }) => void;
...
export function useBridgePolling({
  ...
  useEffect(() => {
    if (pendingTransactionHashes.length > 0) {
      ...
      const timeoutId = setTimeout(() => {
        ...
        const confirmedTx = transactions.find((tx) => tx.txHash === hashToConfirm);
  ...
  }, [pendingTransactionHashes, transactions, updateTransaction, removePendingTransactionHash]);

  // Watch for transaction hash and update pending transaction
  useEffect(() => {
    const currentTxHash = lastTransactionId ? transactions.find(tx => tx.id === lastTransactionId)?.txHash : null;
    if (currentTxHash) {
      addPendingTransactionHash(currentTxHash);
      setLastTransactionId(null);
    }
  }, [lastTransactionId, transactions, addPendingTransactionHash, setLastTransactionId]);
    ...
    if (isTxConfirmed && txReceipt && pendingTransactionHashes.length > 0) {
      ...
      if (confirmedTx) {
        ...
        if (isCrossChain) {
          ...
          updateTransaction(confirmedTx.id, { commandId: commandId || undefined });
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      for (const crossChainTx of crossChainTransactions) {
        try {
          const chainName = getChainNameForGateway(parseInt(crossChainTx.destinationChain));
