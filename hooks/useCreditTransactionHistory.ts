...
export const useCreditTransactionHistory = () => {
  ...
  const saveHistory = useCallback((newHistory: CreditTransactionHistory[]) => {
  ...
  const addTransaction = useCallback((transaction: Omit<CreditTransactionHistory, 'id' | 'timestamp'>) => {
