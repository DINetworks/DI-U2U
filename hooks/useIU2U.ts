...
import { useAccount, useWalletClient, useReadContract, useChainId, useWriteContract, useBalance } from 'wagmi';
...
export function useIU2UContract(chainName?: string): UseIU2UContractReturn {
...
export function useIU2UBalance(chainName?: string): UseIU2UBalanceReturn {
...
export function useIU2UTokenOperations(chainName?: string) {
  const { iu2uToken: tokenAddress } = useIU2UContract(chainName);
...
export function useIU2UGatewayOperations(chainName?: string) {
  const { iu2uGateway: gatewayAddress } = useIU2UContract(chainName);
  const { writeContract, isPending, error } = useWriteContract();
  ...
  const sendToken = useCallback(async (
    ...
    if (!gatewayAddress) throw new Error('IU2U Gateway contract not available');

    return writeContract({
      address: gatewayAddress as `0x${string}`,
      abi: IU2U_GATEWAY_ABI,
      functionName: 'sendToken',
      args: [destinationChain, recipient as `0x${string}`, symbol, BigInt(amount) * BigInt(10 ** 18)],
    });
  ...
  const callContract = useCallback(async (
    ...
    return writeContract({
      ...
      args: [destinationChain, contractAddress as `0x${string}`, payload as `0x${string}`],
    });
  }, [gatewayAddress, writeContract]);

  // Call contract with token transfer
  const callContractWithToken = useCallback(async (
    destinationChain: string,
    contractAddress: string,
    payload: string,
    symbol: string,
...
// Utility function to get chain name from chain ID
